...
let seed=1,g=M.create(seed),spectate=false,filter='',timer;
const names={skip:'守住记忆',power:'回想神明',legacy:'最后馈赠',burn:'燃忆',recall:'寻忆',watch:'守望',offer:'供奉',contest:'改写',rest:'安魂',trim:'放下',end:'交棒',yield:'让出',defend:'守住'};
function node(tag,text,cls){const n=document.createElement(tag);if(text!==undefined)n.textContent=text;if(cls)n.className=cls;return n;}
function apply(id){try{g=M.act(g,M.actor(g),id);M.check(g);render();}catch(e){$('hint').textContent='火塘暂歇：'+e.message;clearTimeout(timer);}}
function render(){clearTimeout(timer);const ended=g.phase==='ended',who=M.actor(g),p=g.players[0];
 $('title').textContent=ended?'天将明，名字留下了什么':`第${g.round}夜 · ${g.pending?'有人要改写供奉':who===0?'轮到你守夜':g.players[who].name+'正在守夜'}`;
 $('hint').textContent=ended?'火塘已落定，下面记录着这一夜真正发生的事。':g.pending?(who===0?'你的供奉正被争夺。付出一张相应记忆守住它，或带着余音让出。':'等待对方回应。'):'四座神座，退去后不再补入。每一处空缺都会留到天明。';
 ...
 $('gods').replaceChildren(...g.seats.map((s,i)=>{const d=M.GODS[i],n=node('article',undefined,'god '+(s.state==='active'?'':s.state==='awake'?'awake':'retired'));n.style.setProperty('--fade',s.state==='ruin'?1:s.state==='awake'?0:s.weather/3);const img=node('img');img.src='../../online/public/art/'+d.image;img.alt=d.name;const body=node('div',undefined,'body');body.append(node('h2',s.state==='ruin'?'□□□':d.name),node('p',({active:`风化 ${s.weather}/3`,awake:'名字重新完整',rest:'已送别',ruin:'已被遗忘'})[s.state]));const slots=node('div',undefined,'slots');for(const e of d.elements){const o=s.offerings.find(o=>o.element===e),slot=node('div',e,'slot');slot.append(node('span',o?g.players[o.player].name:'空缺'));if(o)slot.style.setProperty('--ink',colors[o.player]);slots.append(slot);}body.append(slots,node('p',d.effect));n.append(img,body);return n;}));
 $('hand').replaceChildren(...p.hand.map(c=>node('div',c.god===undefined?c.element:M.GODS[c.god].name+'的记忆','card'+(c.god===undefined?'':' gift'))));
 ...
 $('actions').replaceChildren(...actions.filter(a=>a.type===filter).map(a=>{const b=node('button',a.label);b.onclick=()=>{if(a.type==='burn'&&!confirm('这张记忆将永远离开，你留下1点伤痕（−1分）。仍要燃忆吗？'))return;apply(a.id);};return b;}));
 $('events').replaceChildren(...g.events.slice(-12).reverse().map(e=>node('li',`第${e.round}夜 · ${e.text}`)));
...
$('auto').onclick=()=>{spectate=!spectate;render();};$('restart').onclick=()=>{if(!confirm('熄灭当前实验火塘，重新开始？'))return;g=M.create(++seed);filter='';spectate=false;render();};render();
