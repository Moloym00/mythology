...
const MemoryGame = (() => {
  ...
  function create(seed=1) { const g={version:'M0.1',rng:seed>>>0,round:1,starter:0,turn:0,acted:0,phase:'turn',step:'aux',pending:null,failed:false,events:[],players:[],seats:GODS.map(()=>({state:'active',weather:0,offerings:[]}))};
  ...
  function settle(g) { g.seats.forEach((s,id)=> { if(s.state!=='active'||s.offerings.length!==3)return; const counts=g.players.map((_,i)=>s.offerings.filter(o=>o.player===i).length);const winner=counts.findIndex(n=>n>=2);if(winner>=0){const p=g.players[winner];p.awake.push(id);p.deck.push({id:`god-${id}`,god:id,element:'真名',owner:winner});event(g,'awake',winner,id,`${p.name}唤醒${GODS[id].name}，祂的记忆进入牌堆顶。`);}else event(g,'communal',null,id,`${GODS[id].name}众声觉醒，没有人独得祂的记忆。`);counts.forEach((n,i)=>{if(i!==winner)g.players[i].echo+=n;});release(g,s);s.state='awake'; }); }
  function finishTurn(g) { g.acted++;if(g.acted===3){settle(g);if(g.round===6||!g.seats.some(s=>s.state==='active')){end(g);return;}g.round++;g.starter=(g.starter+1)%3;g.turn=g.starter;g.acted=0;storm(g);if(g.phase==='ended')return;}else g.turn=(g.turn+1)%3;g.step='aux';draw(g,g.turn,1); }
